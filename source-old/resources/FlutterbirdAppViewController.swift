...
class FlutterbirdAppViewController: UIViewController {
  @IBOutlet weak var playButton: UIButton!
  @IBOutlet weak var auContainerView: UIView!
  
  var playEngine: SimplePlayEngine!
  var mAudioUnitViewController: FlutterbirdViewController!
  ...
  override func viewDidLoad() {
    ...
    AUAudioUnit.registerSubclass(IPlugAUAudioUnit.self, as: desc, name:"iPlug: Local Flutterbird", version: UInt32.max)
  ...
  func embedPlugInView() {
    ...
    let pluginURL = builtInPlugInsURL.appendingPathComponent("FlutterbirdAppExtension.appex")
    let appExtensionBundle = Bundle(url: pluginURL)
    
    let storyboard = UIStoryboard(name: "Flutterbird-iOS-MainInterface", bundle: appExtensionBundle)
    mAudioUnitViewController = storyboard.instantiateInitialViewController() as? FlutterbirdViewController
