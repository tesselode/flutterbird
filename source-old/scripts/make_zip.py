...
def  main():
  ...
  installer = "\installer\Flutterbird Installer.exe"
   
  if demo:
    installer = "\installer\Flutterbird Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Flutterbird manual.pdf" 
  ]
  ...
  ZIPNAME = "Flutterbird-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Flutterbird-v" + FULLVERSIONSTR + "-win-demo.zip"
