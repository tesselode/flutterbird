...
if %1 == 1 (echo Making Flutterbird Windows DEMO VERSION distribution ...) else (echo Making Flutterbird Windows FULL VERSION distribution ...)
...
REM - msbuild Flutterbird-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild Flutterbird.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Flutterbird.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Flutterbird.aaxplugin\Contents\Win32\Flutterbird.aaxplugin --out .\build-win\aax\bin\Flutterbird.aaxplugin\Contents\Win32\Flutterbird.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Flutterbird.aaxplugin\Contents\x64\Flutterbird.aaxplugin --out .\build-win\aax\bin\Flutterbird.aaxplugin\Contents\x64\Flutterbird.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Flutterbird.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Flutterbird.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Flutterbird Installer" ".\installer\Flutterbird Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Flutterbird Installer.exe" ".\installer\Flutterbird Demo Installer.exe"
REM -del ".\installer\Flutterbird Installer.exe"
REM -)
